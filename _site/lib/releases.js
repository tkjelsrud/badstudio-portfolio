...
async function loadReleaseData(jsonFile) {
    try {
        ...
        let releaseInfo = `
            ...
            <p><strong>ISIN:</strong> ${release.isin} &copy;</p>
        ...
        releaseInfo += `</ul><p><strong>Chains:</strong></p><ul>`;

        for (const [chain, value] of Object.entries(release.production.chains)) {
            releaseInfo += `<li>${chain}: ${value}</li>`;
        }

        releaseInfo += `
            </ul>
            <p><strong>Author:</strong> ${release.credits.writer}</p>
